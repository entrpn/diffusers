...
# os.environ["TPU_CHIPS_PER_PROCESS_BOUNDS"] = "1"
...
def main(args):
    ...
    print("autocast enabled? ", torch.backends.mps.is_available())
    ...
    train_dataset = dataset["train"].with_transform(compiled_preprocess_train)
    ...
    # Train!
    total_batch_size = args.train_batch_size * xm.xrt_world_size() * args.gradient_accumulation_steps

    print("***** Running training *****")
    print(f"  Num examples = {len(precomputed_dataset)}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            def compute_time_ids(original_size, crops_coords_top_left):
                ...
                print("add_time_ids device: ", add_time_ids.device)
                return add_time_ids
            ...
            pooled_prompt_embeds = batch["pooled_prompt_embeds"].to(device)
            ...
            print("added_cond_kwargs[time_ids]: ", unet_added_conditions["time_ids"].device)
            print("added_cond_kwargs[text_embeds]: ", unet_added_conditions["text_embeds"].device)
            ...
            avg_loss = torch_xla.core.functions.all_gather(loss.repeat(args.train_batch_size)).mean()
