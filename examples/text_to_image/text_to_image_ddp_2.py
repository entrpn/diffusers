...
class  SdModel(torch.nn.Module):
    def __init__(self, vae, unet, text_encoder, noise_scheduler, device, weight_dtype):
...
class TrainSD():
    ...
    def __init__(self, first_epoch, vae, weight_dtype, device, noise_scheduler, unet, optimizer, text_encoder, data_loader, args, rank):
        ...
        self.args = args
        self.rank = rank
        self.model = DDP(SdModel(vae, unet, text_encoder, noise_scheduler, self.device, self.weight_dtype).to(self.device), 
    ...
    def step_fn(
        ...
        with xp.Trace("model_run"):
            model_pred, target, timesteps = self.model(batch["pixel_values"], batch["input_ids"])

        with xp.Trace("compute_loss"):
